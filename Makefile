...
DOCKER_TASKNAME   = piarmy-minio-client
...
shell: stop
	...
	@echo ""
	@docker run -it --rm --network=piarmy --name=$(DOCKER_TASKNAME) $(DOCKER_IMAGE_NAME) /bin/bash

run: stop
	@docker run -d --rm --network=piarmy --name=$(DOCKER_TASKNAME) $(DOCKER_IMAGE_NAME)
